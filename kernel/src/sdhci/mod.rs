...
impl SdhciSlot {
    ...
    fn power_up(&mut self, voltage: SdhciOperatingVoltage) -> Result<(), &'static str> {
        unsafe {
    ...
    // freq in KHz
    fn set_sdclock_frequency(&mut self, freq: u32) -> Result<(), &'static str> {
        unsafe {
    ...
    fn soft_reset(&mut self) -> Result<(), &'static str> {
        unsafe {
